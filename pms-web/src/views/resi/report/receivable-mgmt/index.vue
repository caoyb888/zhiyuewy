  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="80px">
      ...
      <el-form-item label="缴费状态" prop="payState">
        <el-select v-model="queryParams.payState" placeholder="全部" clearable style="width: 120px">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" v-hasPermi="['resi:report:receivableMgmt:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="tableData" border>
      ...
      <el-table-column label="缴费状态" prop="payState" align="center" width="90">
        <template slot-scope="scope">
          <el-tag :type="payStateTagType(scope.row.payState)">{{ payStateLabel(scope.row.payState) }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="批次号" prop="genBatch" min-width="140" :show-overflow-tooltip="true" />
...
<script>
import { formatMoney, feeTypeLabel, payStateLabel, payStateTagType } from '@/utils/resi'
import { receivableMgmt, exportReport } from '@/api/resi/report'
import { listProject } from '@/api/resi/archive'
import { listFeeDefinition } from '@/api/resi/feeconfig'

export default {
  name: 'ResiReceivableMgmt',
  filters: {
    formatMoney
  },
  data() {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      receivableMgmt(this.queryParams).then(response => {
    ...
    handleExport() {
      exportReport('/resi/report/receivable-mgmt', this.queryParams, '应收管理报表')
    }
  }
}
</script>
